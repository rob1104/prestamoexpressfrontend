<template>
  <q-dialog v-model="mostrar" persistent>
    <q-card style="width: 450px; max-width: 95vw;" class="shadow-24 bg-grey-2">
      <q-card-section class="bg-indigo-9 text-white row items-center q-pb-sm">
        <div class="text-h6 text-weight-bold">
          <q-icon name="point_of_sale" size="sm" class="q-mr-sm" />
          Aceptar Pago de Electrónicos
        </div>
        <q-space />
...
const form = ref({ efectivo: 0, credito: 0, debito: 0, importe_separado: 0 })
...
const confirmarCobro = () => {
  if (totalRecibido.value <= 0) return $q.notify({ type: 'negative', message: 'NO se aceptan Pagos en CERO' })

  if (props.tipoVenta === 'SEPARADO') {
    if (form.value.importe_separado < minimoSeparado.value) return $q.notify({ type: 'negative', message: 'El Importe de Separado es MENOR del 20% del Total a Pagar' })
    if (totalRecibido.value < form.value.importe_separado) return $q.notify({ type: 'negative', message: 'El Importe de Separado es MAYOR al Total Recibido' })
  } else {
    if (totalRecibido.value < props.totalPagar) return $q.notify({ type: 'negative', message: 'El Importe Recibido es MENOR al Total a Pagar' })
  }

  // Validaciones del Modo de Venta vs Entradas
  if (props.modo === 'CONTADO' && (form.value.credito > 0 || form.value.debito > 0)) return $q.notify({ type: 'negative', message: 'El Importe Capturado NO concuerda con el Modo de Venta' })
  if (props.modo === 'VOUCHER' && (form.value.efectivo > 0 || form.value.debito > 0)) return $q.notify({ type: 'negative', message: 'El Importe Capturado NO concuerda con el Modo de Venta' })
  if (props.modo === 'DEBITO' && (form.value.efectivo > 0 || form.value.credito > 0)) return $q.notify({ type: 'negative', message: 'El Importe Capturado NO concuerda con el Modo de Venta' })
  if (props.modo === 'MIXTO' && form.value.efectivo === 0 && form.value.credito === 0 && form.value.debito === 0) return $q.notify({ type: 'negative', message: 'El Importe Capturado NO concuerda con el Modo de Venta' })

  if ((props.modo === 'VOUCHER' || props.modo === 'DEBITO') && totalRecibido.value > montoRequerido.value) {
    return $q.notify({ type: 'negative', message: 'En transacciones de Tarjeta el Importe Recibido NO puede ser mayor a la Venta' })
  }
...
<style scoped>
  ...
  :deep(.q-field--focused .q-field__control) { background-color: #e0e7ff !important; }
</style>
